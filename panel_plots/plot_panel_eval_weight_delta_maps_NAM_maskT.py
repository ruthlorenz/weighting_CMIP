...
for d in range(0, nrow):
    ...
    for o in range(0, ncol):
        path_mm = '%s/plot_delta_mm_%s%s_JJA_latlon_%s_%s_%s_RMSE_%s_%s.nc' %(
        ...
        path_wmm = '%s/plot_delta_wmm_%s%s_JJA_latlon_%s_%s_%s_RMSE_%s_%s.nc' %(
            indir, variable, diag, dnum, obsdata[o], region, wu, wq)
        print path_wmm
...
colbar='RdBu_r'
levels = [-0.55, -0.45, - 0.35, -0.25, -0.15, -0.05, 0.05, 0.15,
          0.25, 0.35, 0.45, 0.55]
